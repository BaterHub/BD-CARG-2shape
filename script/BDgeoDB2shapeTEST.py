...
class CARGProcessor:
    ...
    def get_foglio_from_geodatabase(self):
        ...
        # Scorri tutti i dataset e layer
        for dataset_name, layer_list in available_layers_dict.items():
            ...
            for layer_name in layer_list:
                try:
                    # Costruisci il percorso completo del layer
    ...
    def handle_direzione_field_for_geology_polygons_fixed(self, shapefile):
        """
        FIXED version - Manage Direzione field without overwriting with zeros
        Uses FeatureClassToFeatureClass instead of AlterField for better compatibility
        """
        arcpy.AddMessage("Handling Direzione field for geologia_poligoni (FIXED v2)...")
        ...
        try:
            ...
            # If we only have Direzio, we need to rename it to Direzione
            if direzio_field and not direzione_field:
                try:
                    # METHOD 1: Try direct field rename (this might fail on shapefiles)
                    arcpy.AlterField_management(shapefile, direzio_field, "Direzione", "Direzione")
                    arcpy.AddMessage("Successfully renamed field {} to Direzione".format(direzio_field))
                    return
                    
                except Exception as rename_error:
                    arcpy.AddMessage("Direct rename failed: {}".format(str(rename_error)))
                    arcpy.AddMessage("Using alternative method...")
                    
                    # METHOD 2: Create temp shapefile with field mappings
                    try:
                        workspace_dir = os.path.dirname(shapefile)
                        temp_name = "temp_direzione_" + os.path.basename(shapefile)
                        ...
                        # Add all fields, renaming Direzio to Direzione
                        for field in arcpy.ListFields(shapefile):
                            if field.type not in ['OID', 'Geometry']:
                                field_map = arcpy.FieldMap()
                                field_map.addInputField(shapefile, field.name)
                                
                                # Rename Direzio to Direzione
                                if field.name == direzio_field:
                                    output_field = field_map.outputField
                                    output_field.name = "Direzione"
                                    output_field.aliasName = "Direzione"
                                    field_map.outputField = output_field
                                    arcpy.AddMessage("Mapping {} -> Direzione".format(field.name))
                                
                                field_mappings.addFieldMap(field_map)
                        
                        # Create new shapefile with corrected field names
                        ...
                        arcpy.Rename_management(temp_path, shapefile)
                        
                        arcpy.AddMessage("Successfully renamed field using FeatureClassToFeatureClass method")
                        return
                        
                    except Exception as mapping_error:
                        arcpy.AddWarning("FeatureClassToFeatureClass method failed: {}".format(str(mapping_error)))
                        ...
                        # METHOD 3: Fallback - add new field and copy values
                        arcpy.AddMessage("Using fallback method - add new field and copy values...")
                        ...
                        if direzio_field_obj:
                            field_type = direzio_field_obj.type
                            field_length = getattr(direzio_field_obj, 'length', 50) if field_type == 'String' else None
                            
                            # Add Direzione field
                            if field_type == 'String':
                                arcpy.AddField_management(shapefile, "Direzione", "TEXT", field_length=field_length)
                            else:
                                arcpy.AddField_management(shapefile, "Direzione", field_type)
                            
                            arcpy.AddMessage("Added Direzione field with type {}".format(field_type))
                        else:
                            # Fallback field type
                            arcpy.AddField_management(shapefile, "Direzione", "DOUBLE")
                            arcpy.AddMessage("Added Direzione field with default type DOUBLE")
                        
                        # Copy values from Direzio to Direzione (preserve original values)
                        copied_count = 0
                        null_count = 0
                        ...
                        with arcpy.da.UpdateCursor(shapefile, [direzio_field, "Direzione"]) as cursor:
                            for row in cursor:
                                ...
                                if direzio_val is not None:
                                    # Keep the original value - don't convert to 0!
                                    cursor.updateRow([direzio_val, direzio_val])
                                    copied_count += 1
                                else:
                                    # Set None for null values (or empty string for text fields)
                                    if direzio_field_obj and direzio_field_obj.type == 'String':
    ...
    def debug_t2000_mapping_issues_fixed(self, shapefile, auxiliary_tables):
        ...
        # FIXED: Case-insensitive search for UC_LEGE field
    ...
    def process_all_optimized(self):
        ...
        try:
            ...
            if not available_layers:
                raise RuntimeError("No layers found in input GeoDB!")
